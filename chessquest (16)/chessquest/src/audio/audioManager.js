...
import { trackFor, namedTracks } from './musicRegistry.js';
...
export class AudioManager {
  ...
  /**
   * Advance to the next custom music track (the in-game "switch track" button).
   * Loads it directly by dir/file, bypassing the per-theme registry mapping, so
   * the player can listen to any track during any match. Returns the new title.
   */
  cycleMusic() {
    const tracks = namedTracks();
    if (!tracks.length) return '';
    // Find current index by title, advance, wrap.
    const idx = tracks.findIndex((t) => t.title === this._currentTrackTitle);
    const next = tracks[(idx + 1) % tracks.length];
    this._loadTrackDirect(next);
    return next.title;
  }

  _loadTrackDirect(track) {
    this._musicContext = `manual:${track.title}`;
    if (this._music) {
      try { this._music.fade(this._music.volume(), 0, 400); } catch { /* ignore */ }
      const old = this._music;
      setTimeout(() => { try { old.unload(); } catch { /* ignore */ } }, 500);
      this._music = null;
      this._musicId = null;
    }
    this._currentTrackTitle = track.title;
    ...
    this._music = new Howl({
      ...
      onplay: () => { if (this._musicId != null) this._music.fade(0, this._musicVolume, 800, this._musicId); },
      onloaderror: () => { this._music = null; },
    });
    this._wantMusic = true;
    if (this._musicEnabled !== false && !this._muted) {
      this._musicId = this._music.play();
    }
  }
