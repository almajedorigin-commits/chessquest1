...
function buildSummary({ category, tags, moveSan, swingCp, bestLineBefore, before, mover }) {
  ...
  if (category === 'best' || category === 'good') {
    if (tags.includes('hanging-piece')) {
      return `${moveSan} is fine materially for now, but double-check that piece next move -- it's currently undefended.`;
    }
    return `${moveSan} matches what the engine considers the strongest continuation here. Solid choice.`;
  }

  const lossNote = bestSanHint ? ` ${bestSanHint} was stronger.` : '';

  if (tags.includes('hanging-piece')) {
    return `${moveSan} leaves a piece undefended where your opponent can simply take it for free.${lossNote}`;
  }

  if (category === 'inaccuracy') {
    return `${moveSan} is a small slip -- not losing, but it gives away a bit of your advantage.${lossNote}`;
  }
  if (category === 'mistake') {
    return `${moveSan} is a real mistake -- it hands your opponent a meaningful edge.${lossNote}`;
  }
  if (category === 'blunder') {
    return `${moveSan} is a serious blunder -- it likely costs material or the game outright.${lossNote}`;
  }
